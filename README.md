...
- Material Design 3 界面，**固定品牌配色**（靛蓝 / 薄荷 / 淡紫，与图标同源），
  支持名称/包名搜索、用户/系统应用筛选和已启用规则置顶；
...
| 用途 | 色值 | Material 角色 |
| --- | --- | --- |
| 主色（靛蓝） | `#4E4CC9` | `colorPrimary` |
| 强调（薄荷） | `#62E3BD` | `colorTertiary` |
| 次级（淡紫） | `#8F8BFF` | `colorSecondary` |
| 图标背景 | `#17145C` | 自适应图标背景层 |

两点需要说明：

- **不启用动态取色**。`DynamicColors.applyToActivitiesIfAvailable` 已注释掉——
  它会用系统壁纸派生色覆盖上述品牌配色。若要改回跟随系统，取消该行注释即可；
- **启动器图标不随明暗主题变化**。设计稿要求深靛背景配浅薰衣草图形，
  浅底上会几乎不可见，因此背景层固定为 `#17145C`。
  在开启主题化图标（themed icons）的启动器上，则使用单色图层并由系统着色。
